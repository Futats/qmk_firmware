...
enum keycode_aliases {
  ...
  SY_PIPE = LGUI_T(KC_PIPE),
  SY_PERC = LALT_T(KC_PERC),
  SY_LT = LCTL_T(KC_LT),
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	...
	[SYM] = LAYOUT_split_3x5_3(
        KC_GRV, 		KC_QUOT, 	LSFT(KC_QUOT), 	LSFT(KC_4), LSFT(KC_1),			LSFT(KC_EQL), 	LSFT(KC_LBRC), 	LSFT(KC_RBRC), 	LSFT(KC_SLSH), 	KC_PAST,
		SY_PIPE, 		SY_PERC, 	SY_LT, 			SY_EQL, 	KC_GT,				KC_MINS, 		SY_LPRN, 		SY_RPRN, 		SY_UNDS, 		SY_COLN,			
		LSFT(KC_GRV), 	LSFT(KC_3), LSFT(KC_6), 	LSFT(KC_7), KC_BSLS,			LSFT(KC_2), 	KC_LBRC, 		KC_RBRC, 		KC_PDOT, 		KC_PSLS,				
									KC_TRNS, 		KC_TRNS, 	KC_TRNS,			KC_TRNS, 		KC_TRNS, 		KC_TRNS
    ),

	[NUM] = LAYOUT_split_3x5_3(
		KC_F1, 	KC_F2, 	KC_F3, 	KC_F4, 	KC_HOME,						LSFT(KC_EQL), 	KC_P7, 	KC_P8, 	KC_P9, 	KC_PAST,
		N_F5, 	N_F6,	N_F7,	N_F8, 	KC_END,							KC_PMNS, 		N_4, 	N_5, 	N_6, 	N_COLN,
		KC_F9, 	KC_F10, KC_F11, KC_F12, KC_CALC,						KC_P0, 			KC_P1,	KC_P2,	KC_P3,	KC_PSLS,
						KC_LSFT, KC_LCTL, KC_LOPT,						KC_PDOT, 		KC_P0, 	KC_PDOT
	),
...
// We can't send non-basic taps by default, so we need to intercept our complex 
// tap-hold keys and send the more sophisticated keys here
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (record->tap.count && record->event.pressed) {
        switch (keycode) {
            case SY_PIPE: tap_code16(KC_PIPE);  return false;
            case SY_PERC: tap_code16(KC_PERC);  return false;
            case SY_LT:   tap_code16(KC_LT);    return false;
            case SY_EQL:  tap_code16(KC_EQL);   return false;
            case SY_COLN: tap_code16(KC_COLN);  return false;
